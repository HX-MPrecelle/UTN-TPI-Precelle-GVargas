...
# Configurar la salida para usar UTF-8 (necesario en Windows)
...
def ejecutar_filtro_por_poblacion():
    ...
    try:
        ...
        if poblacion_min is None or poblacion_max is None:
            print("❌ Debe ingresar parámetros válidos en ambos casos")
            pausar_ejecucion()
            return
...
def ejecutar_agregar_pais():
    """Ejecuta la agregación de un nuevo país."""
    global paises
    ...
    try:
        ...
        paises_existentes = buscar_pais_por_nombre(paises, nombre, busqueda_exacta=True)
        ...
        # Solicitar población
        poblacion = validar_entrada_numero(
            input("Ingrese la población: "), 1, 2000000000
        )
        if poblacion is None:
            print("❌ La población debe ser un número válido mayor a 0")
            pausar_ejecucion()
            return
        
        # Solicitar superficie
        superficie = validar_entrada_numero(
            input("Ingrese la superficie (km²): "), 1, 20000000
        )
        if superficie is None:
            print("❌ La superficie debe ser un número válido mayor a 0")
            pausar_ejecucion()
            return
        ...
        continentes = obtener_continentes_disponibles(paises)
        mostrar_continentes_disponibles(continentes)
        print(f"{len(continentes) + 1}. Ingresar nuevo continente")
        
        try:
            opcion = int(input(f"\nSeleccione un continente (1-{len(continentes) + 1}): "))
            if 1 <= opcion <= len(continentes):
                continente = continentes[opcion - 1]
        ...
        print(f"❌ Error al agregar país: {e}")
    
    pausar_ejecucion()


def ejecutar_actualizar_pais():
    """Ejecuta la actualización de datos de un país."""
    global paises
    ...
    try:
        ...
        # Buscar país (búsqueda exacta)
        paises_encontrados = buscar_pais_por_nombre(paises, nombre, busqueda_exacta=True)
        ...
        nueva_poblacion = input(f"Población actual: {pais['poblacion']} → Nueva población: ").strip()
        if nueva_poblacion:
            poblacion = validar_entrada_numero(nueva_poblacion, 1, 2000000000)
            if poblacion is not None:
                pais['poblacion'] = poblacion
            else:
                print("⚠️ Población inválida, se mantiene el valor actual")
        
        # Actualizar superficie
        nueva_superficie = input(f"Superficie actual: {pais['superficie']} → Nueva superficie (km²): ").strip()
        if nueva_superficie:
            superficie = validar_entrada_numero(nueva_superficie, 1, 20000000)
    ...
    while True:
        try:
            ...
            if opcion == 0:
                ...
                ejecutar_agregar_pais()
            elif opcion == 2:
                ejecutar_actualizar_pais()
            elif opcion == 3:
